...
export const LintImportsAfterEditPlugin: Plugin = async ({
  $,
  directory,
  client,
}) => {
  ...
  const createToolLog = () => {
    return (
      ...
      writeLog({
        directory,
        body: {
          service: "LintImportsAfterEditPlugin",
          level,
          message,
          extra: {
            ...extra,
  ...
  return {
    "tool.execute.after": async (input, output) => {
      ...
      traceLog("Run task check:imports...");
      // NOTE: Taskを経由してlint-importsを実行
      const result = await $`task check:imports`.nothrow().quiet(); // ⚠️CAUTION: .quiet()がないとOpenCodeのTUIがおかしくなる

      traceLog("task check:imports complete.", {
        extra: { tool, result },
      });
      ...
      if (result.exitCode === 0) {
        ...
        await client.tui.showToast({
          body: {
            message: "lint imports has finished successfully",
            variant: "success",
          },
        });
      ...
      await client.tui.showToast({
        body: {
          message: "Lint imports failed; sending errors back to the agent",
          variant: "error",
        },
      });
      ...
      await client.session.prompt({
        ...
        body: {
          ...
          parts: [
            {
              type: "text",
              text: `Fix these lint imports errors:\n${errors}`,
            },
          ],
        },
      });
